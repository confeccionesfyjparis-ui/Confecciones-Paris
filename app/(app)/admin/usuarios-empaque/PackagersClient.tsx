...
import { createPackagerUser, togglePackagerActive, resetPackagerPassword, deletePackagerUser } from "@/lib/actions/users";

type PackagerUser = { id: string; username: string; active: boolean };
type SummaryRow = { id: string; number: string; garment_name: string; initial_qty: number; total_packaged: number };

export function PackagersClient({
  initialUsers,
  summary,
}: {
  initialUsers: PackagerUser[];
  summary: SummaryRow[];
}) {
  ...
  function addUser() {
    ...
    startTransition(async () => {
      try {
        const res = await createPackagerUser(username, password);
  ...
  function savePassword(id: string) {
    ...
    startTransition(async () => {
      try {
        await resetPackagerPassword(id, resetPassword);
  ...
  return (
    <div>
      <div className="text-[15px] font-semibold text-[var(--navy)] mb-2">Usuarios de empaque</div>
      <div className="bg-[#EEF1E9] border border-[#D3DAC4] text-[#4B5540] rounded-lg px-3.5 py-2.5 text-sm mb-4 leading-relaxed">
        Estos usuarios registran cuántas prendas de cada orden/referencia van quedando empacadas
        — es un conteo libre, no valida contra la producción registrada.
      </div>
      ...
      <div className="text-[15px] font-semibold text-[var(--navy)] mb-3">Resumen de empaque por orden</div>
      {summary.length === 0 ? (
        <div className="text-center text-sm text-[var(--muted)] py-6">Todavía no se ha registrado ningún empaque.</div>
      ) : (
        <div className="flex flex-col gap-2">
          {summary.map((s) => (
            <div key={s.id} className="app-card flex justify-between items-center px-3.5 py-3">
              <div className="text-sm font-semibold">{s.number} · {s.garment_name}</div>
              <div className="text-sm text-[var(--muted)]">{s.total_packaged} / {s.initial_qty} empacadas</div>
            </div>
          ))}
        </div>
      )}
